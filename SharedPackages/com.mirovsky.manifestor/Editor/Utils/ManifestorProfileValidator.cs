    ...
    public static class ManifestorProfileValidator
    {
        ...
        private static void ValidatePackageLists(IReadOnlyList<PackagesListSO> packageLists, List<string> errors)
        {
            ...
            for (var listIndex = 0; listIndex < packageLists.Count; listIndex++)
            {
                ...
                ValidateScopedRegistries(packageList, owner, registryOwners, errors);
            }
        }
            ...
            List<string> errors)
        {
            ...
            for (var packageIndex = 0; packageIndex < packageList.packages.Count; packageIndex++)
            {
                ...
                if (string.IsNullOrEmpty(StringUtils.Normalize(package.location)))
                {
            ...
            List<string> errors)
        {
            ...
            for (var registryIndex = 0; registryIndex < packageList.scopedRegistries.Count; registryIndex++)
            {
                ...
                var registryName = StringUtils.Normalize(registry.scopeName);
                if (string.IsNullOrEmpty(registryName))
                {
                    errors.Add($"Scoped registry at index {registryIndex} in {owner} has an empty name.");
                }

                if (string.IsNullOrEmpty(StringUtils.Normalize(registry.scopeUrl)))
                {
                ...
                for (var scopeIndex = 0; scopeIndex < registry.scopes.Count; scopeIndex++)
                {
                    if (string.IsNullOrEmpty(StringUtils.Normalize(registry.scopes[scopeIndex])))
                    {
                        errors.Add($"Scope at index {scopeIndex} in scoped registry '{registryName}' in {owner} is empty.");
                    }
                }

                var normalizedScopes = registry.scopes
                    .Select(StringUtils.Normalize)
                    .Where(scope => !string.IsNullOrEmpty(scope))
                    .Distinct(StringComparer.Ordinal)
                    .OrderBy(scope => scope, StringComparer.Ordinal);
                var registryKey = registryName + "\n" + StringUtils.Normalize(registry.scopeUrl) + "\n" +
                                  string.Join("\n", normalizedScopes);
                if (registryOwners.TryGetValue(registryKey, out var existingOwner))
                {
                    errors.Add($"Scoped registry '{registryName}' in {owner} is already declared in {existingOwner}.");
                }
                else
                {
                    registryOwners.Add(registryKey, owner);
                }
            }
        }
        ...
        {
            return errors.Select(error => $"- {error}");
        }
    }
}
